...
export async function buildSystemPrompt(params: SessionParams, uiContext?: ChatUiContext): Promise<string> {
  ...
  const lines = [
    ...
    `For deck files, always pass paths relative to the Deck Source root (e.g. "index.html", "assets/cover.png"). The deck's on-disk location is an internal detail that may change between sessions, so don't try to reconstruct an absolute path to it — relative paths are resolved correctly by every file tool.${skillsBlock ? ' Skill files (listed below) are the only exception: read them via the absolute path their <location> tag gives.' : ''}`,
