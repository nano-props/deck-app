 ...
 */
export async function recordOpen(entry: { path: string; name: string }): Promise<void> {
  const resolved = path.resolve(entry.path)
  const current = await readRaw()
  const existing = current.findIndex((e) => path.resolve(e.path) === resolved)
...
export async function forgetRecent(p: string): Promise<void> {
  const resolved = path.resolve(p)
  const current = await readRaw()
  const filtered = current.filter((e) => path.resolve(e.path) !== resolved)
