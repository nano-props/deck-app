...
| `titleBarStyle` | Traffic lights                                | Overlay |
| --------------- | --------------------------------------------- | ------- |
| `hiddenInset`   | Visible, centered in the 32px topbar via math | n/a     |
...
| `titleBarStyle` | Caption buttons     | Overlay                                        |
| --------------- | ------------------- | ---------------------------------------------- |
| `hidden`        | Visible, right side | Theme-adaptive solid (`overlayForTheme`), 32px |
...
| `titleBarStyle` | Caption buttons | Overlay                |
| --------------- | --------------- | ---------------------- |
| `hidden`        | Depends on WM   | Same config as Windows |
...
The `LINUX` strategy in `chrome-strategy.ts` is a **field-for-field copy** of `WIN` (not a reference — the explicit copy ensures future edits touch Linux as a separate, visible step). The topbar itself is `-webkit-app-region: drag`, so dragging works as long as the topbar paints; if `titleBarOverlay` fails to render the caption buttons on a given WM, the window can still be moved by its topbar but min/max/close won't be reachable.
...
2. **Ship a minimal custom titlebar.** Render close/min/max buttons inside the topbar. Most involved; needs theme awareness.
...
**Practical implication:** the deck has the full viewport to itself — there is no reserved dead-zone inside the deck page. The persistent topbar owns the OS drag region (it's `-webkit-app-region: drag`), so author pages can put `position: fixed; top: 0` headers, right-aligned controls, etc. anywhere in the deck viewport without colliding with window chrome.
